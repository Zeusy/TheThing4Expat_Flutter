...
import 'package:flutter_app_test/authentification/profil_page.dart';
...
class _PageContentState extends State<PageContent> {
  ...
  _myPage(int currentPage) {
    switch (currentPage) {
      case 0: {
        return ProfilPage();
        //return Center(child: Text("${tr("current")}: ${FirstScreen.mainMenu[currentPage].title}"));
      }
      break;
      default : {
        return Center(child: Text("${tr("current")}: ${FirstScreen.mainMenu[currentPage].title}"));
  ...
  Widget build(BuildContext context) {
    ...
    return PlatformScaffold(
      ...
      appBar: PlatformAppBar(
        ...
        title: PlatformText(
          FirstScreen.mainMenu[_currentPage].title,
        ),
      ...
      bottomNavBar: PlatformNavBar(
        ...
        items: FirstScreen.mainMenu.map((item) => BottomNavigationBarItem(
          title: Text(
            item.title,
            style: style,
          ),
